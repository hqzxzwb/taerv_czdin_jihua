...
- [在线辞典](https://hqzxzwb.github.io/taerv_czdin_jihua/) [旧版主页](https://hqzxzwb.github.io/taerv_czdin_jihua/home.html) [SourceForge在线辞典](https://taerv.sourceforge.io/)
...
- 文件组织方案
  ...
  - 项目中现有两种词条格式，均可使用。编辑既有词条时，应保持该词条及其周边文件的既有风格一致，不要在同一词条内混用两种格式。
  ...
  - 旧格式中，以`>`作为参考文献起头，以`-`作为义项起头。

  - 新格式中，以`*`作为义项下的参考信息起头，以`+`作为义项起头。通常`*`后接参考文献；`* 通用`是特例，表示该义项在各地普遍使用，只作用于当前义项，不代表整个词条。

  - `* 通用`应放在对应义项下，且最好放在各参考文献之前。通用的例子挂在`* 通用`下面；某一参考书专有的例子挂在该参考书下面。

  - 若一个词条有义项标注`* 通用`，则生成页面时会为当前支持自动换算的所有地区生成 IPA。未标`* 通用`的既有义项，即使没有来源/地区标记，暂不视为通用。
